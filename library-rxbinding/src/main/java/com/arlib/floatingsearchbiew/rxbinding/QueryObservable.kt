...
class QueryObservable @JvmOverloads constructor(private val view: FloatingSearchView, private val minQueryLength: Int = 1) : InitialValueObservable<CharSequence>() {
    ...
    class Listener(private val view: FloatingSearchView, private val observer: Observer<in CharSequence>, private val minQueryLength: Int) : MainThreadDisposable(), FloatingSearchView.OnQueryChangeListener {
        override fun onSearchTextChanged(oldQuery: String, newQuery: String) {
            if (!isDisposed && newQuery.length >= minQueryLength) {
